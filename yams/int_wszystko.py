...
def int_wszystko(Ca, Cepsilon, dd, dip_pos ,kL, Lambda, RBx,RBz,xk,zk,ME,MM,TE,TM,settings):
    ...
    if dd>0: # there is a layer beneeth the dipole
        ...
        if type(kL[dd-1]==np.ndarray): # integration is on the dipole inner layer
            ...
            (zperp,zpara)=st*em_int(wspEx,wspMx,kd,RBx[dd-1],xp2inv,a_el,a_ml, Da_el,cc4,cc3,Cepsilon[dd],settings)
            ...
            (zperp,zpara)=st*em_int(wspEz,wspMz,kz,RBz[dd-1],xp2inv,a_el,a_ml, Da_el,cc4,cc3,Cepsilon[dd-1],settings)
    else: # integrate 2 nm beneeth dipole, if necessary
        if (dip_pos>=5 and np.sum(np.abs(a_el.imag))>0):
            ...
            (zperp,zpara)=st*em_int(wspEx,wspMx,kd,RB,xp2inv,a_el,a_ml, Da_el,cc4,cc3,Cepsilon[dd],settings)
...
#    *xp2inv**2 * np.matmul(d_el,cc4)).real # [L x 1]
    MRadPerp = (3/2*(np.absolute(Cepsilon[dd])/Cepsilon[-1])\
    *xp2inv**2 * sum_conv( d_el*cc4.swapaxes(0,1),1,settings)).real # [L x 1]
    
#    MRadPara =(3/4*(np.absolute(Cepsilon[dd])/Cepsilon[-1])\
#    *xp2inv * np.matmul(d_ml+Dd_el,cc3)).real # [L x 1]
    MRadPara =(3/4*(np.absolute(Cepsilon[dd])/Cepsilon[-1])\
    *xp2inv * sum_conv( (d_ml+Dd_el)*cc3.swapaxes(0,1),1,settings)).real # [L x 1]
    ...
    if dd<nK: # there is outer layer
        ...
        if type(kL[dd+1]==np.ndarray): # outer layer is gold, which means
            ...
            (xperp,xpara)=-st*em_int(wspEz,wspMz,kd,RBz[dd],xp2inv,d_el,d_ml,Dd_el,cc4,cc3,Cepsilon[dd],settings)
            ...
            (xperp,xpara)=-st*em_int(wspEx,wspMx,kx,RBx[dd],xp2inv,d_el,d_ml,Dd_el,cc4,cc3,Cepsilon[dd+1],settings)
...
#    sio.savemat('test.mat',matlab_dict)
    ...
    return (MRadPerp[:,0], MRadPara[:,0],MNRPerp[:,0],MNRPara[:,0],MTotPerp[:,0],MTotPara[:,0])  
 
def em_int(wspE,wspM,k,RB,xp2inv,d_el,d_m,Dd_el,cc4,cc3,Cepsilon,settings):
    fi_el=(wspE[0]*RB[2] + wspE[1]*RB[0])/k # L x N
    Dfi_el=(wspE[0]*RB[3] + wspE[1]*RB[1])/k # L x N
    fi_m=(wspM[0]*RB[2] + wspM[1]*RB[0])/k # L x N
    Dfi_m=(wspM[0]*RB[3] + wspM[1]*RB[1])/k # L x N
    # cc4  N x 1
#    int_perp = xp2inv**2* np.matmul( np.nan_to_num(-Dfi_el*fi_el.conj()*d_el) ,cc4)
    int_perp = xp2inv**2* sum_conv(np.nan_to_num(-Dfi_el*fi_el.conj()*d_el)*cc4.swapaxes(0,1), 1,settings)
#    int_para = xp2inv*np.matmul(np.nan_to_num(-Dfi_el*fi_el.conj()*Dd_el+fi_m*Dfi_m.conj()*d_m),cc3)
    int_para = xp2inv* sum_conv(np.nan_to_num(-Dfi_el*fi_el.conj()*Dd_el+fi_m*Dfi_m.conj()*d_m)*cc3.swapaxes(0,1),1,settings)
